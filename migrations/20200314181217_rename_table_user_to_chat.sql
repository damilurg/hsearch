...
alter table user rename to chat;
...
insert into chat_dg_tmp(username, id, enable) select username, chat, enable from chat;
...
-- +goose Down
-- +goose StatementBegin
alter table chat rename to user;
...
insert into user_dg_tmp(chat, username, enable) select id, username, enable from user;
